...
export class InstallManager {
    ...
    static async uninstall(sender:WebContents, communityPackageDir: string, packageCacheDirs: string): Promise<boolean | Error> {
    ...
    static setupIpcListeners(): void {
        ...
        ipcMain.handle(channels.installManager.uninstall, async (event, communityPackageDir: string, packageCacheDir: string) =>{
            return InstallManager.uninstall(event.sender, communityPackageDir, packageCacheDir);
        });
    }
}