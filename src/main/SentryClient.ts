...
export class SentryClient {
    static initialize(): void {
        ...
        let dsn;
        try {
            const extraResourcePath = process.env.NODE_ENV === 'production' ? path.join(process.resourcesPath, 'extraResources') : 'extraResources';
            dsn = fs.readdirSync(path.join(extraResourcePath, '.sentrydsn'), {encoding: 'utf-8'});
        } catch (e) {
